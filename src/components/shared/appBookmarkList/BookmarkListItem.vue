...
export default {
  ...
  methods: {
    likeItem() {
      console.log(this._userLikes)
      const likes = [...this._userLikes, this.item.id]
      this.$appAxios.patch(`users/${this._getCurrentUser.id}`, {likes}).then(response => {
  ...
  computed:{
    ...
    alreadyLiked(){
      return {
        "bookmark-item-active": this._userLikes?.includes(this.item.id)
      }
    },
